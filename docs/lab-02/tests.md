...
| UI-04 | UI | AC-06 | Submitting a valid form when the create API rejects (mocked) | Error message shown; all field values still present in inputs | `client/.../lab-02/CreateTicket.test.tsx` | Planned |
| UI-05 | UI | AC-07 | Requester with 0 tickets opens My Tickets (mocked empty response) | "Empty" state with Create Ticket CTA, not "No results" | `client/.../lab-02/MyTickets.test.tsx` | Planned |
| UI-06 | UI | AC-08 | Requester with tickets applies a filter matching none (mocked) | "No results" state with Clear Filters CTA | `client/.../lab-02/MyTickets.test.tsx` | Planned |
| UI-07 | UI | AC-10 | Mocked Requester switch while My Tickets is open | Old list cleared, new Requester's tickets fetched and shown | `client/.../lab-02/MyTickets.test.tsx` | Planned |
| UI-08 | UI | AC-12 | Selecting a `.exe` file in the attachment picker | Rejected client-side with message; no upload request sent | `client/.../lab-02/AttachmentSection.test.tsx` | Planned |
| UI-09 | UI | AC-16 | Requester Selection screen with 0 active requesters (mocked) | Safe empty state, not a blank/broken dropdown | `client/.../lab-02/RequesterSelection.test.tsx` | Planned |
| UI-10 | UI | AC-17 | Requester Selection screen when `GET /api/requesters` fails (mocked) | Safe error state, not a crash or infinite spinner | `client/.../lab-02/RequesterSelection.test.tsx` | Planned |
